...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $amt_p = $_POST['amt_p'];
    $sql3 = "SELECT receipt_number FROM payment ORDER BY id DESC LIMIT 1";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($w_amt < $amt_p){
            echo "Insufficient balance in your wallet. Click <a href='payapp.php'>here </a> to go back or <a href='logapp.php'>Cancel Payment</a>";
        } else{
        $sql = "INSERT INTO payment (paid_for, reg_num, course_id, invoice_number, receipt_number, f_amount, t_fee, t_amount, sch_session, p_status)
                VALUES ('Application Form', '$username', '$course_id', '$inv', '$newReceiptNumber', '$f_amount', '$t_fee', '$amt_p', '$section', 'PAID')";
