...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Fetch the lecturer's assigned course codes
    $sql = "SELECT course_code FROM users WHERE username = '$username'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        // Split the comma-separated course codes into an array
        $assigned_courses = explode(',', $row['course_code']);
        // Check if the selected course code is assigned to the lecturer
        if (in_array($code, $assigned_courses)) {
    ...
    if ($stmt->execute()) {
        ...
        // Display error message if the lecturer is not authorized to add scores for the selected course
        echo "You do not have access to add scores for the selected course code.";
    }
